...
void BTF::display_funcs(void) const
{
  ...
  for (id = 1; id <= max; id++)
  {
    ...
    if (!btf_is_func_proto(t))
    {
      /* bad.. */
      if (!bt_verbose)
        std::cerr << "ERROR: " << func_name << " function does not have FUNC_PROTO record" << std::endl;
      break;
    }
    ...
    for (j = 0, p = btf_params(t); j < btf_vlen(t); j++, p++)
    {
      ...
      if (err)
      {
        std::cerr << "ERROR: failed to dump argument: " << arg_name << std::endl;
        break;
      }
  ...
  if (id != (max + 1))
    std::cerr << "ERROR: BTF data inconsistency " << id << "," << max << std::endl;
...
int BTF::resolve_args(std::string &func __attribute__((__unused__)),
                      std::map<std::string, SizedType> &args __attribute__((__unused__)),
